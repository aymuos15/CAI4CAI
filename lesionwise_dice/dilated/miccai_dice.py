...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    if challenge_name == 'BraTS-GLI':
        ...
        dilation_factor = 0 # SET TO ZERO FOR EXAMPLE SAKE
    ...
    for labels in range(len(label_values)):
        ...
        tp, fn, fp, gt_tp, metric_pairs, full_dice = get_LesionWiseScores(
                                                    prediction_seg = pred_file,
                                                    gt_seg = gt_file,
                                                    label_value = label_values[labels],
                                                    dil_factor = dilation_factor
                                                )
        ...
        metric_df['Label'] = [label_values[labels]]*len(metric_df)
        ...
        final_metrics_dict[label_values[labels]] = metrics_dict
    ...
    return results_df, lesion_wise_dice