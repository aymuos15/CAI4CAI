...
#### Below this is all the original code from florian's repo
    ...
    for element in range(batch_length):
        ...
        for ula in unique_labels:
            if ula == 0:
                ...
                ax[0].imshow(the_label[0, 0, ...])
                ax[0].set_title("Current Component")
                ax[1].imshow(masked_output[0, 0, ...].detach().cpu().numpy())
                ax[1].set_title("Prediction")
                ax[2].imshow(label_mask[0, 0, ...])
                ax[2].set_title("Mask")
                ax[3].imshow(the_label[0, 0, ...] + masked_output[0, 0, ...])
